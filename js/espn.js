...
var ESPNSync = (function() {
  ...
  function parseTeams(data) {
    ...
    // Find user's team by matching SWID to owner
    data.teams.forEach(function(team) {
      var isMe = false;
      if (data.members) {
        var member = data.members.find(function(m) {
          return ('{' + m.id + '}') === S.espn.swid || m.id === S.espn.swid.replace(/[{}]/g, '');
        });
        if (member) {
          isMe = team.owners && team.owners.includes(member.id);
        }
      }
      // Fallback: check team owners against SWID
      if (!isMe && team.owners) {
        isMe = team.owners.some(function(oid) {
          return oid === S.espn.swid.replace(/[{}]/g, '');
        });
      }
    ...
    if (data.members) {
      data.members.forEach(function(m) {
        S.teams.forEach(function(t) {
          ...
          if (rawTeam && rawTeam.owners && rawTeam.owners.includes(m.id)) {
            t.owner = (m.displayName || m.firstName + ' ' + m.lastName || '').trim();
            if (t.teamId === S.myTeam.teamId) S.myTeam.owner = t.owner;
          }
        });
      });
    }
  ...
  async function syncAll() {
    try {
      ...
      if (data) {
        ...
        addSyncLog('success', 'Synced ' + S.allPlayers.length + ' players');
